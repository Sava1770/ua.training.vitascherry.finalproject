...
public interface StudentProgressQuery {
    ...
    String FIND_ALL = "SELECT id_student, first_name, last_name, patronymic, id_quiz, quiz.name, " +
                          "(SELECT COUNT(id_question) question " +
                          "WHERE id_quiz = que.id_quiz) " +
                          "AS question_count, SUM(ans.is_correct) AS correct_count " +
                      ...
                      "GROUP BY id_student, first_name, last_name, patronymic, id_quiz, quiz.name";

    String FIND_BY_STUDENT_ID = "SELECT id_student, first_name, last_name, patronymic, id_quiz, quiz.name, " +
                                    "(SELECT COUNT(id_question) question " +
                                    "WHERE id_quiz = que.id_quiz) " +
                                    "AS question_count, SUM(ans.is_correct) AS correct_count " +
                                "FROM student_answer AS stud_ans " +
