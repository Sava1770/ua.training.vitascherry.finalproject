SELECT id_student, email, first_name, last_name, patronymic, id_quiz, quiz.name, (SELECT COUNT(id_question) question WHERE id_quiz = que.id_quiz) AS question_count, SUM(ans.is_correct) AS correct_count
